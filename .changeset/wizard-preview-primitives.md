...
- **`@openzeppelin/ui-components/code-view`** — a read-only syntax-highlighted code pane with no character-size cutoff. An optional `decorateToken` callback lets you customize how individual highlighted tokens render; omit it for standard highlighting with no added decoration. The subpath also exports `CODE_VIEW_LANGUAGES` and the `isCodeViewLanguage` type guard, so a host holding a language id that came from data rather than from a literal can narrow it to a grammar `CodeView` renders instead of casting and silently getting the plaintext fallback.
