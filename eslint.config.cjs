...
/**
 * Extract plugin rules from different config formats
 */
function getPluginConfigs(plugin, configName) {
  // Check flat config format first (configs property)
  if (plugin.configs && plugin.configs[configName]) {
    const config = plugin.configs[configName];
    // Handle array format (like react-hooks)
    if (Array.isArray(config)) {
      return config.reduce((acc, item) => ({ ...acc, ...(item.rules || {}) }), {});
    }
    return config.rules || {};
  }
  return {};
