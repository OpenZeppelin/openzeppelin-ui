...
- Local development against consumer repos MUST be supported via the shared `oz-dev` CLI, which bootstraps `.openzeppelin-dev.json`, config-driven `.pnpmfile.cjs` hooks, and managed `dev:local` / `dev:npm` scripts.
