...
function getTargetPackages(dependencies: string[]): string[] {
  const knownFamilyPackages = new Set(
    Object.values(STANDARD_FAMILIES)
...
function resolvePackageSources(context: TailwindProjectContext, cssDirectory: string): string[] {
  const packageSources = new Set<string>();

  for (const packageName of getTargetPackages(context.dependencies)) {
 ...
 */
export function buildTailwindSourcePlan(context: TailwindProjectContext): TailwindSourcePlan {
  const cssDirectory = path.dirname(context.generatedCssPath);
  const packages = getTargetPackages(context.dependencies);
  const appSources = ['./', '../'];
  const workspaceSources = resolveWorkspaceSources(context, cssDirectory);
  const packageSources = resolvePackageSources(context, cssDirectory);
  const imports = [
