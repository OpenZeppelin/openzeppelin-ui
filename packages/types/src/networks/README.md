...
This document explains the pattern for defining network configurations in the OpenZeppelin UI ecosystem of packages.

## Overview

The OpenZeppelin UI ecosystem supports multiple blockchain ecosystems (EVM, Solana, Stellar, Midnight) with various networks (mainnet, testnets, etc.) within each ecosystem. To maintain a chain-agnostic architecture while preserving type safety and separation of concerns, we use a pattern where:
...
3. The application discovers and aggregates these configurations
...
### 4. Application Network Discovery

The application dynamically discovers and aggregates network configurations from all registered adapter packages. This is achieved by:
...
2. Dynamically importing the main module of each adapter package.
3. Accessing the exported network list from the imported module using the conventional name (e.g., `module.evmNetworks`).

This allows the application to remain decoupled from the specifics of each adapter package while still being able to gather all network configurations.

```typescript
// Simplified conceptual example from ecosystemManager.ts
...
async function loadAdapterPackageModule(ecosystem: Ecosystem): Promise<any> {
  switch (ecosystem) {
    case 'evm':
      return import('@your-org/adapter-evm');
    case 'solana':
      return import('@your-org/adapter-solana');
    // ... etc.
    default:
      throw new Error('...');
  }
...
4. **Chain-Agnosticism**: Application remains agnostic, only knowing about networks through adapters
