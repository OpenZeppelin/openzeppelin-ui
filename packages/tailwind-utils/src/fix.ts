...
export function printTailwindProject(
  ...
  return {
    ...
    sourcePlan: buildTailwindSourcePlan(project, families),
  };
}
...
export function fixTailwindProject(
  ...
  if (!project) {
    return {
      ...
      generatedCssPath: null,
      sourcePlan: null,
      issuesBefore: doctorTailwindProject(projectRootInput, families, branding, options.cssPath)
        .issues,
      changes: [],
      changed: false,
      wrote: false,
  ...
  const generatedCssContent = createManagedTailwindCss(sourcePlan, branding);
  ...
  const currentGeneratedCssContent = fs.existsSync(project.generatedCssPath)
    ...
    : null;
  const issuesBefore = doctorTailwindProject(
    project.projectRoot,
    families,
    branding,
    options.cssPath
