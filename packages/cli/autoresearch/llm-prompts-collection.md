...
**Before running:** Generate a scaffold first with `npx tsx autoresearch/scaffold-expected.ts <FIXTURE_NAME>`. This creates `expected/<FIXTURE_NAME>.scaffold.json` as a starting point for the agent to refine.
...
Read the scaffold at `packages/cli/autoresearch/expected/<FIXTURE_NAME>.scaffold.json` —
it was generated by a dumb regex scanner and has false positives and blind spots.
