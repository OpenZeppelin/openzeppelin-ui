...
**Run the evaluation:**
```bash
npx tsx autoresearch/evaluate.ts
```
...
**Current baseline:** mean_f1 ≈ 0.7556 (raw-html: 1.0, shadcn: 1.0, radix: 0.267)
...
Append each experiment line to `autoresearch/results.tsv`. The file has no header — just data rows.

Before your first experiment, if `results.tsv` does not exist, create it empty.
...
The biggest gap is **Radix namespace imports**. The current `extractImports` function only handles `import { X } from '...'` and `import X from '...'` — it does NOT handle `import * as X from '...'`. The Radix fixture uses exclusively namespace imports (`import * as Tabs from '@radix-ui/react-tabs'`) with JSX like `<Tabs.Root>`. Solving this requires:
1. Detecting `import * as X from '...'` patterns
2. Matching JSX usage like `<X.SubComponent>` back to the namespace import
3. Mapping the namespace name (e.g., `Tabs`) to the correct OZ target
