...
/**
 * Registers the bare `oz-ui create [project-name]` scaffolding action on the
 * already-created `create` command. Keep this distinct from `oz-ui create init`,
 * which lives in `./init.ts` and only installs skill assets.
 */
export function registerScaffoldAction(create: Command): void {
  create
